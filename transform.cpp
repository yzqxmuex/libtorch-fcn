...
//ReduceMean
ReduceMean::ReduceMean(std::tuple<float, float, float> mean)
	: mean_(mean) {}
torch::Tensor ReduceMean::operator()(torch::Tensor input) {
	float bMean = std::get<0>(mean_);
	float gMean = std::get<1>(mean_);
	float rMean = std::get<2>(mean_);

	return input = input.sub((bMean, gMean, rMean));
}
