...
class ReduceMean : public torch::data::transforms::TensorTransform<torch::Tensor>
{
public:
	//Create a transformation that reduce meam a tensor
	explicit ReduceMean(std::tuple<float, float, float> mean);
	torch::Tensor operator()(torch::Tensor input) override;

private:
	std::tuple<float, float, float> mean_;
};
}  // namespace transform
